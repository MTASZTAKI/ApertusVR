...
ApeHelloWorldPlugin::ApeHelloWorldPlugin()
{
	...
	if (auto planetNode = mPlanetNode.lock())
	{
		...
		{
			std::stringstream mediaFolderPath;
			mediaFolderPath << APE_SOURCE_DIR << "/samples/helloWorld/plugin/resources";
			sceneProperty->addResourceLocation(mediaFolderPath.str(), THIS_PLUGINNAME);
