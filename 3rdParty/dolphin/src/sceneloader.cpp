...
std::vector<Dolphin::SceneComponent*>* DotSceneLoader::GetSceneComponents()
{
...
void DotSceneLoader::processEntity(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	try
	{
		...
		pEntity->getMesh()->buildEdgeList();
		unsigned short src, dest;
		src = 0;
		dest = 0;


//		if (!pEntity->getMesh()->suggestTangentVectorBuildParams(Ogre::VES_TANGENT, src, dest))
//		{
//			pEntity->getMesh()->buildTangentVectors(Ogre::VES_TANGENT, src, dest);
//		}
		...
		Dolphin::RenderableSceneComponent* scenecomponent = new Dolphin::RenderableSceneComponent();
		scenecomponent->SetNode(pParent);
		scenecomponent->SetModel(pEntity);
		scenecomponent->SetPhysicsStatus(true);
...
void DotSceneLoader::processLightAttenuation(TiXmlElement *XMLNode, Light *pLight)
{
	...
	pLight->setAttenuation(range, 1.0f, 4.5 / range, 75.0f / (range*range));
}


String DotSceneLoader::getAttrib(TiXmlElement *XMLNode, const std::string &attrib, const String &defaultValue)
{
...
Real DotSceneLoader::getAttribReal(TiXmlElement *XMLNode, const std::string &attrib, Real defaultValue)
{
	if (XMLNode->Attribute(attrib.c_str()))
