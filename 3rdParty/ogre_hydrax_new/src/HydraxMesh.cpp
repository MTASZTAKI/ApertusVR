...
namespace Hydrax
{
    ...
    void Mesh::setOptions(const Options& Options)
    {
		if (mCreated)
		{
			...
			{
				meshBounds = Ogre::AxisAlignedBox(0, -Options.MeshStrength/2, 0, Options.MeshSize.Width, Options.MeshStrength/2, Options.MeshSize.Height);
			}

      mMesh->_setBounds(meshBounds);
      mMesh->_setBoundingSphereRadius(std::max(meshBounds.getMaximum().x - meshBounds.getMinimum().x, std::max(meshBounds.getMaximum().y - meshBounds.getMinimum().y, meshBounds.getMaximum().z - meshBounds.getMinimum().z))/2.0f);
		    ...
		    case VT_POS_NORM_UV:
			{
				...
				mVertexBuffer = Ogre::HardwareBufferManager::getSingleton().
			        createVertexBuffer(sizeof(POS_NORM_UV_VERTEX),
			                           numVertices,
			                           Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE, true);
			}
			break;
    ...
    if (use32bitIndexes)
    {
		  ...
		  for(int v=0; v<Complexity-1; v++)
      {
        ...
        {
				  // face 1 |/
				  pIdx[i++] = v*Complexity + u;
				  pIdx[i++] = v*Complexity + u + 1;
				  pIdx[i++] = (v+1)*Complexity + u;

				  // face 2 /|
				  pIdx[i++] = (v+1)*Complexity + u;
				  pIdx[i++] = v*Complexity + u + 1;
				  pIdx[i++] = (v+1)*Complexity + u + 1;
			  }
		  }
