...
namespace Hydrax{namespace Module
{
    ...
    bool use32bitIndexes = (numVertices > 65535) ? true : false;
		...
		// Vertex buffers
    Ogre::SubMesh* mSubMesh = mMesh->getSubMesh();
      ...
      for (int k = 0; k < mOptions.Steps; k++)
		  ...
		  for(int y=0; y<mOptions.Circles-1; y++) 
		  {
		    for(int x=0; x<mOptions.Steps; x++) 
			  {
			      Ogre::uint32* twoface = pIdx + (y*mOptions.Steps+x)*6 + 3 * mOptions.Steps;
    ...
    else
    {
      ...
      for (int k = 0; k < mOptions.Steps; k++)
		  ...
		  for(int y=0; y<mOptions.Circles-1; y++) 
		  {
		    for(int x=0; x<mOptions.Steps; x++) 
			  {
			      Ogre::uint16* twoface = pIdx + (y*mOptions.Steps+x)*6 + 3 * mOptions.Steps;
	...
	void RadialGrid::update(const Ogre::Real &timeSinceLastFrame, const Ogre::Camera* renderingCamera)
	{
		if (!isCreated())
		{
			return;
