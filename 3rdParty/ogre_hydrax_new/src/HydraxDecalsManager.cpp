...
namespace Hydrax
{
  ...
  void DecalsManager::update(const Ogre::Camera* cam)
  {
    if (cam->getDerivedPosition() == mLastPosition &&
      cam->getDerivedOrientation() == mLastOrientation &&
      !mForceToUpdate)
    {
      return;
    }
    ...
    mLastPosition = cam->getDerivedPosition();
    mLastOrientation = cam->getDerivedOrientation();
