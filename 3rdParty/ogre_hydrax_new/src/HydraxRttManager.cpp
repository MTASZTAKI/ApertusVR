...
namespace Hydrax
{
  RttManager::RttManager(Hydrax *h)
  {
    ...
    mReflectionDisplacementError = 0.005f;
  ...
  void RttManager::remove(const RttType& Rtt)
  {
    ...
    if (!Tex.isNull())
    {
      ...
      // Drop the last reference to the texture, Ogre::Texture destructor will be called.
      Tex.setNull();
  ...
  void RttManager::updateAll()
  {
    for (int k = 0; k < 6; k++)
    {
      ...
      if (!Tex.isNull())
      {
        ...
        buf->getRenderTarget()->update();
      }
    }
  }

  void RttManager::setAutoUpdate(bool state)
  {
    mAutoUpdate = state;
    for (int k = 0; k < 6; k++)
    {
      ...
      {
        Ogre::HardwarePixelBufferSharedPtr buf = Tex->getBuffer();
        buf->getRenderTarget()->setAutoUpdated(state);
      }
    }
  }

  bool RttManager::isAutoUpdated()
  {
    return mAutoUpdate;
  }

  bool RttManager::isRttViewport(const Ogre::Viewport* viewport)
  {
    bool found = false;
    for (int k = 0; k < 6 && !found; k++)
    {
      ...
      {
        Ogre::HardwarePixelBufferSharedPtr buf = Tex->getBuffer();
        if ((buf->getRenderTarget()->getNumViewports() > 0) && (viewport == buf->getRenderTarget()->getViewport(0)))
          found = true;
      }
    }
    ...
    const bool& ShadowsEnabled)
  {
    ...
    Ogre::MeshManager::getSingleton().
      createPlane(mRttOptions[Rtt].Name + "ClipPlane", HYDRAX_RESOURCE_GROUP,
        *mPlanes[Rtt],
    ...
    if (TSize.Width == 0 && TSize.Width == 0)
    {
      ...
      if (lviewports.size() > 0)
        viewport = (*lviewports.begin());

      if (viewport)
      {
        TSize.Width = viewport->getActualWidth();
        TSize.Height = viewport->getActualHeight();
      }
    }
    ...
    Ogre::Real oldRatio = mHydrax->getCamera()->getAspectRatio();
    Ogre::Viewport *RT_Texture_Viewport = RT_Texture->addViewport(mHydrax->getCamera());
    mHydrax->getCamera()->setAspectRatio(oldRatio);
    RT_Texture_Viewport->setVisibilityMask(mHydrax->getRttVisibilityMask());
    ...
    RT_Texture->setAutoUpdated(mAutoUpdate);
    if (MaterialScheme != "")
    {
      RT_Texture_Viewport->setMaterialScheme(MaterialScheme);
    }
    RT_Texture_Viewport->setSkiesEnabled(true);
  ...
  void RttManager::CReflectionListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
  {
    ...
    // Underwater
    if (mHydrax->_isCurrentFrameUnderwater() &&
      (mHydrax->getCamera()->getDerivedPosition().y > mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->_getDerivedPosition().y))
    {
      mCameraPlaneDiff = 0;
    ...
    // Overwater
      ...
      (mHydrax->getCamera()->getDerivedPosition().y < mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->_getDerivedPosition().y))
    {
      mCameraPlaneDiff = mRttManager->mPlanes[RTT_REFLECTION]->getParentNode()->_getDerivedPosition().y - mHydrax->getCamera()->getDerivedPosition().y + mRttManager->mReflectionDisplacementError;
    ...
    while (EntityIterator.hasMoreElements())
    {
      ...
      for(k = 0; k < CurrentEntity->getNumSubEntities(); k++)
      {
        ...
        if (SubEntMat)
        {
          ...
          {
            Ogre::Technique* tec = static_cast<Ogre::Technique*>(TechIt.peekNext());
            if (tec->getSchemeName() == "HydraxDepth")
            {
                DepthTechniquePresent = true;
            }
            else
            {
              //ignore techniques with depth write off
              if(tec->getNumPasses() > 0)
              {
                //ignore skyx materials, transparent materials, and alpha rejection
                if((SubEntMat->getName().find("SkyX_") == Ogre::String::npos) && !tec->getPass(0)->getDepthWriteEnabled() || (tec->getPass(0)->getAlphaRejectValue() > 120.0f))
                  DepthTechniquePresent = true;
              }
            }
        ...
        Ogre::MaterialPtr mat = mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_DEPTH);
        if (mat)
          CurrentEntity->getSubEntity(k)->setMaterialName(mat->getName());
      }
...
*/
    ...
    {
      mHydrax->getMesh()->getEntity()->setVisible(true);
      Ogre::MaterialPtr mat = mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_SIMPLE_RED);
      if (!mat.isNull())
        mHydrax->getMesh()->getEntity()->setMaterialName(mat->getName(), mat->getGroup());
  ...
  void RttManager::CDepthListener::postRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
  {
    ...
    */
    ...
    Ogre::MaterialPtr mat = mHydrax->getMaterialManager()->getMaterial(mHydrax->_isCurrentFrameUnderwater() ? MaterialManager::MAT_UNDERWATER : MaterialManager::MAT_WATER);
    if (!mat.isNull())
      mHydrax->getMesh()->getEntity()->setMaterialName(mat->getName(), mat->getGroup());
    ...
    if (Ogre::Math::Abs(mHydrax->getPosition().y - mHydrax->getCamera()->getDerivedPosition().y) > mHydrax->getPlanesError())
    {
      if (mHydrax->_isCurrentFrameUnderwater())
      {
        Ogre::MaterialPtr mat = mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_UNDERWATER);
        if (!mat.isNull())
          mHydrax->getMesh()->getEntity()->setMaterialName(mat->getName(), mat->getGroup());
  ...
  void RttManager::CDepthReflectionListener::preRenderTargetUpdate(const Ogre::RenderTargetEvent& evt)
  {
    ...
    bool DepthTechniquePresent = false;
      ...
      for(k = 0; k < CurrentEntity->getNumSubEntities(); k++)
      {
        ...
        Ogre::MaterialPtr mat = mHydrax->getMaterialManager()->getMaterial(MaterialManager::MAT_DEPTH);
        if (mat)
          CurrentEntity->getSubEntity(k)->setMaterialName(mat->getName(), mat->getGroup());
      }
    ...
    */
    ...
    if (mHydrax->getCamera()->getDerivedPosition().y > mRttManager->mPlanes[RTT_DEPTH_REFLECTION]->getParentNode()->_getDerivedPosition().y)
    {
      mCameraPlaneDiff = 0;
      IsInUnderwaterError = true;
    }
