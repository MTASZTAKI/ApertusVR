...
namespace Hydrax{namespace Noise
{
    ...
    FragmentProgramData += Ogre::String("void main_fp(\n") +
		...
		mMaterialManager->fillGpuProgramsToPass(Technique0_Pass0, GpuProgramNames, EntryPoints, GpuProgramsData);
		...
		Technique0_Pass0->createTextureUnitState(mGPUNormalMapManager->getTexture(0)->getName(), 0)->setTextureAddressingMode(Ogre::TextureUnitState::TAM_WRAP);
	...
	{
		unsigned short *Data;
		Ogre::HardwarePixelBufferSharedPtr PixelBuffer = mGPUNormalMapManager->getTexture(0)->getBuffer();
