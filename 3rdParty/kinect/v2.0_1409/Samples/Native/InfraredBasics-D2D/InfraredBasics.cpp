...
LRESULT CALLBACK CInfraredBasics::DlgProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    switch (message)
    {
        case WM_INITDIALOG:
        {
            ...
            HRESULT hr = m_pDrawInfrared->Initialize(GetDlgItem(m_hWnd, IDC_VIDEOVIEW), m_pD2DFactory, cInfraredWidth, cInfraredHeight, cInfraredWidth * sizeof(RGBQUAD)); 
            if (FAILED(hr))
            {
...
/// </returns>
HRESULT CInfraredBasics::GetScreenshotFileName(_Out_writes_z_(nFilePathSize) LPWSTR lpszFilePath, UINT nFilePathSize)
{
    ...
    if (SUCCEEDED(hr))
    {
        ...
        // File name will be KinectScreenshotInfrared-HH-MM-SS.bmp
        StringCchPrintfW(lpszFilePath, nFilePathSize, L"%s\\KinectScreenshot-Infrared-%s.bmp", pszKnownPath, szTimeString);
    }
