...
// Changed from 99999, cause I'm using a metric system
#define _def_MaxFarClipDistance 9999.0f
...
namespace Hydrax
{
  namespace Module
  {
    ...
    void ProjectedGrid::remove()
    {
      ...
      mLastHydraxPosition = Ogre::Vector3::ZERO;
    }
    ...
    void ProjectedGrid::update(const Ogre::Real &timeSinceLastFrame, const Ogre::Camera* renderingCamera)
    {
      if (!isCreated())
        return;

      Module::update(timeSinceLastFrame);
      Ogre::Vector3 RenderingCameraPos = renderingCamera->getDerivedPosition();
      ...
      if (mLastPosition != RenderingCameraPos ||
        mLastOrientation != renderingCamera->getDerivedOrientation() ||
        mLastHydraxPosition != HydraxPos ||
        ...
        mForceUpdate)
      {
        ...
        //float RenderingFarClipDistance = renderingCamera->getFarClipDistance();
        //if ((RenderingFarClipDistance == 0) || (RenderingFarClipDistance > _def_MaxFarClipDistance))
          //renderingCamera->setFarClipDistance(_def_MaxFarClipDistance);

        mLastMinMax = _getMinMax(renderingCamera, &mRange);
        if (mLastMinMax)
        {
          _renderGeometry(renderingCamera, mRange, mProjectingCamera->getViewMatrix(), RenderingCameraPos);
          mHydrax->getMesh()->updateGeometry(mOptions.Complexity*mOptions.Complexity, mVertices);
        }

        //renderingCamera->setFarClipDistance(RenderingFarClipDistance);
        mForceUpdate = false;
      }
      else if (mLastMinMax)
      {
        ...
        if (getNormalMode() == MaterialManager::NM_VERTEX)
        {
          ...
          if (mOptions.ChoppyWaves && mOptions.ChoppyStrength != 0.0f)
          {
            for (int i = 0; i < mOptions.Complexity*mOptions.Complexity; i++)
            {
        ...
        _calculeNormals();
        _performChoppyWaves(renderingCamera);
        mHydrax->getMesh()->updateGeometry(mOptions.Complexity*mOptions.Complexity, mVertices);
      }
    ...
    bool ProjectedGrid::_renderGeometry(const Ogre::Camera* renderingCamera, const Ogre::Matrix4& m, const Ogre::Matrix4& _viewMat, const Ogre::Vector3& WorldPos)
    {
    ...
    void ProjectedGrid::_performChoppyWaves(const Ogre::Camera* renderingCamera)
    {
      if (getNormalMode() != MaterialManager::NM_VERTEX || !mOptions.ChoppyWaves || mOptions.ChoppyStrength == 0.0f)
        return;

      int v, u, Underwater = 1;
      ...
      CameraDir = renderingCamera->getDerivedDirection();
    ...
    bool ProjectedGrid::_getMinMax(const Ogre::Camera* renderingCamera, Ogre::Matrix4 *range)
    {
      ...
      mTmpRndrngCamera->setFrustumOffset(renderingCamera->getFrustumOffset());
      mTmpRndrngCamera->setAspectRatio(renderingCamera->getAspectRatio());
      mTmpRndrngCamera->setDirection(renderingCamera->getDerivedDirection());
      mTmpRndrngCamera->setFarClipDistance(std::min(renderingCamera->getFarClipDistance(), _def_MaxFarClipDistance));
      mTmpRndrngCamera->setFOVy(renderingCamera->getFOVy());
      mTmpRndrngCamera->setNearClipDistance(std::max(0.01f, renderingCamera->getNearClipDistance()));
      mTmpRndrngCamera->setOrientation(renderingCamera->getDerivedOrientation());
      mTmpRndrngCamera->setPosition(0, renderingCamera->getDerivedPosition().y - mHydrax->getPosition().y, 0);
      mTmpRndrngCamera->setCustomProjectionMatrix(renderingCamera->isCustomProjectionMatrixEnabled(), renderingCamera->getProjectionMatrix());
      ...
      mProjectingCamera->setCustomProjectionMatrix(mTmpRndrngCamera->isCustomProjectionMatrixEnabled(), mTmpRndrngCamera->getProjectionMatrix());
