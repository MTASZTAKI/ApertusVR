...
float4 normal_mapPS(VertexOutput IN,
            ...
            uniform sampler2D SpecularColorSampler : register(s2),
            uniform sampler2D RoughnessSampler : register(s3),
            uniform sampler2D HeightSampler : register(s4),
            uniform sampler2D AOsampler : register(s5),
            uniform samplerCUBE ReflectionCubeSampler : register(s6)            
) : COLOR

{
    ...
    float3 finalcolor = AmbiColor*(texCol.rgb) +
        att*((texCol.rgb + reflCubeTexture)*SurfaceColor*diff+specularcomponent);
