#include "apeOpenVRPlugin.h"

ape::OpenVRPlugin::OpenVRPlugin()
{
	APE_LOG_FUNC_ENTER();
	mpCoreConfig = ape::ICoreConfig::getSingletonPtr();
	mpEventManager = ape::IEventManager::getSingletonPtr();
	mpEventManager->connectEvent(ape::Event::Group::NODE, std::bind(&OpenVRPlugin::eventCallBack, this, std::placeholders::_1));
	mpEventManager->connectEvent(ape::Event::Group::CAMERA, std::bind(&OpenVRPlugin::eventCallBack, this, std::placeholders::_1));
	mpEventManager->connectEvent(ape::Event::Group::TEXTURE_MANUAL, std::bind(&OpenVRPlugin::eventCallBack, this, std::placeholders::_1));
...
{
	APE_LOG_FUNC_ENTER();
	mpEventManager->disconnectEvent(ape::Event::Group::NODE, std::bind(&OpenVRPlugin::eventCallBack, this, std::placeholders::_1));
	APE_LOG_FUNC_LEAVE();
}

ape::Matrix4 ape::OpenVRPlugin::conversionFromOpenVR(vr::HmdMatrix34_t ovrMatrix34)
{
	ape::Matrix4 matrix4(
...
ape::Matrix4 ape::OpenVRPlugin::conversionFromOpenVR(vr::HmdMatrix44_t ovrMatrix44)
{
	ape::Matrix4 matrix4(
...
void ape::OpenVRPlugin::submitTextureLeftToOpenVR()
{
...
void ape::OpenVRPlugin::submitTextureRightToOpenVR()
{
	//for openVR the only chanche to submit and render the compositor if you doing everything
	//in only one CB function from the render thread which currently is ape::apeOpenVRPlugin::submitTextureLeftToOpenVR() function
}

void ape::OpenVRPlugin::eventCallBack(const ape::Event& event)
{
	if (event.type == ape::Event::Type::TEXTURE_MANUAL_GRAPHICSAPIID)
	{
		if (auto textureManual = std::static_pointer_cast<ape::IManualTexture>(mpSceneManager->getEntity(event.subjectName).lock()))
		{
			if (event.subjectName == "OpenVrRenderTextureLeft")
			{
				...
				textureManual->registerFunction(std::bind(&OpenVRPlugin::submitTextureLeftToOpenVR, this));
			}
			else if (event.subjectName == "OpenVrRenderTextureRight")
			{
				mOpenVrRttTextureIDs[1] = textureManual->getGraphicsApiID();
				textureManual->registerFunction(std::bind(&OpenVRPlugin::submitTextureRightToOpenVR, this));
			}
		}
	}
}

void ape::OpenVRPlugin::Init()
{
	APE_LOG_FUNC_ENTER();
...
void ape::OpenVRPlugin::Run()
{
	...
	if (mpCoreConfig->getWindowConfig().renderSystem == "DX11")
	{
		...
		//TODO_apeOpenVRPlugin bounds for DX11 and crash beacuse of no shader for DX11 texture RTSS?
		/*mOpenVrTextureBounds[0].uMin = 0;
		mOpenVrTextureBounds[0].uMax = 1;
		mOpenVrTextureBounds[0].vMin = 1;
		mOpenVrTextureBounds[0].vMax = 0;
	...
	while (true)
	{
		...
		//compositor and pose update is done by the ape::apeOpenVRPlugin::submitTextureLeftToOpenVR() function called from the rendering thread
		std::this_thread::sleep_for(std::chrono::milliseconds(1));
	}
	APE_LOG_FUNC_LEAVE();
}

void ape::OpenVRPlugin::Step()
{
	APE_LOG_FUNC_ENTER();
	APE_LOG_FUNC_LEAVE();
}

void ape::OpenVRPlugin::Stop()
{
	APE_LOG_FUNC_ENTER();
	APE_LOG_FUNC_LEAVE();
}

void ape::OpenVRPlugin::Suspend()
{
	APE_LOG_FUNC_ENTER();
	APE_LOG_FUNC_LEAVE();
}

void ape::OpenVRPlugin::Restart()
{
	APE_LOG_FUNC_ENTER();
	APE_LOG_FUNC_LEAVE();
}
